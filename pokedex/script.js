...
const setaEsquerda = [document.querySelector('.seta-esquerda-div'), document.querySelector('.setas-pokemon-esquerda')]
const setaDireita = [document.querySelector('.seta-direita-div'), document.querySelector('.setas-pokemon-direita')]
...
fetchPokemon()
        ...
        setaEsquerda[0].addEventListener('click', () =>{
            if (idNumber == 1) return
            idNumber -= 1
            fetchPokemon()
        })
        
        setaDireita[0].addEventListener('click', () =>{
            if (idNumber == 649) return
            idNumber += 1
            fetchPokemon()
        })

        setaEsquerda[1].addEventListener('click', () =>{
            if (idNumber == 1) return
            idNumber -= 1
            fetchPokemon()
        })
        
        setaDireita[1].addEventListener('click', () =>{
            if (idNumber == 649) return
            idNumber += 1
            fetchPokemon()
        })
